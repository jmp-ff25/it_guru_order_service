...
- **OpenAPI JSON**: http://localhost:8075/openapi.json
...
#### Почему SQL-скрипты, а не pg_dump?

**Для тестовых данных SQL-скрипты предпочтительнее:**

✅ **Преимущества SQL-скриптов:**
- **Читаемость**: Код легко просматривать и понимать
- **Версионирование**: Git показывает изменения в данных
- **Редактирование**: Любой редактор, простые изменения
- **Переносимость**: Работает на любой версии PostgreSQL
- **Code Review**: Можно проверить логику тестовых данных

❌ **Недостатки pg_dump для тестовых данных:**
- Бинарные файлы не читаются людьми
- Git не показывает изменения в данных
- Нужны специальные утилиты для просмотра/редактирования
- Зависимость от версии PostgreSQL

#### Когда использовать каждый подход

| Тип данных | Рекомендуемый подход | Причина |
|------------|---------------------|---------|
| **Справочники** (роли, статусы) | SQL-скрипты | Часто изменяются, нужен контроль версий |
| **Тестовые данные** | SQL-скрипты | Читаемость, простота редактирования |
| **Demo данные** | SQL-скрипты | Презентабельность, контролируемость |
| **Копия продакшена** | pg_dump | Объем, скорость, точность |
| **Backup/Restore** | pg_dump | Официальный инструмент PostgreSQL |
| **Миграция между серверами** | pg_dump | Гарантированная совместимость |

#### Промышленные стандарты

В реальных проектах используют **гибридный подход**:
- **Laravel**: `database/seeders/` (PHP классы)
- **Django**: `fixtures/` (JSON/YAML) + `management/commands/`
- **Ruby on Rails**: `db/seeds.rb` (Ruby код)
- **Node.js**: Custom SQL scripts или ORM seeders
- **Java Spring**: `data.sql` или `@Sql` аннотации
...
Тестовое задание IT Guru (Учебный проект)
